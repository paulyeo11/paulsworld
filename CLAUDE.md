...
- **Section moved to top + given a T-number (2026-07-23):** Paul asked to move the whole section to the top of `tools.html` (now **"A. IBKR Update Log"**, first section after Featured, with the other sections re-lettered B–G) and to label the card **"T45: IBKR Update Log — All Statements"** (tnum circle shows `45`) so it matches the numbered-card look of the rest of the hub. **T45 is a display label only — there is no `T45-*.html` file**, the card still links to `ibkr-update-log.html`. Because of this, the next real numbered tool page should be **T46**, not T45, when following the "Investment Tools T07 and up" rule's `ls T*.html` numbering check below.
