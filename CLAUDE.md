...
- **Trigger phrases — treat all the same way:** "check the IBKR folder in Drive", "update my IBKR raw data", and "update IBKR data". On any of these: look up the latest file(s) in that Drive folder via the Google Drive MCP tools, read the raw data, and update the relevant dashboard(s)/positions data from it (currently Table 30 = `options-breadboard.html`).
...
## Table 30 Options Dashboard — Standing Notes (added 2026-07-23)
...
- **Vercel free-plan daily deployment cap — 100/day (confirmed 2026-07-24, see AIT14):** Vercel's free plan hard-caps at **100 deployments per day**. Once hit, any new push (auto-deploy or manual "Create Deployment") fails with `Resource is limited — try again in 24 hours (more than 100, code: api-deployments-free-per-day)`, and the site stays frozen on its last successful deployment until the cap resets ~24h later. This is what causes the "deployment stuck / pushes not showing up live" symptom (see AIT10/AIT12/AIT14) — it isn't a random freeze, it's this quota. Nothing is lost — every push is still safely on GitHub, just queued — and nothing (script, git push, retry) can force a deploy through early. Because so many automated jobs on this site already deploy on their own (price alerts, daily backups, Tiger refreshes) on top of manual pushes, this cap can get hit purely from normal daily activity. If a fresh push doesn't go live and CLAUDE.md's other Vercel-freeze causes don't fit, check for this error first via Vercel dashboard → Deployments → latest attempt, and tell Paul plainly rather than retrying — Netlify is also unusable as a fallback until 16 Aug 2026 (see above), so there is currently no live mirror while this cap is in effect.
