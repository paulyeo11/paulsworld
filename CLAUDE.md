...
- **Trigger phrases — treat all the same way:** "check the IBKR folder in Drive", "update my IBKR raw data", and "update IBKR data". On any of these: look up the latest file(s) in that Drive folder via the Google Drive MCP tools, read the raw data, and update the relevant dashboard(s)/positions data from it (currently T29 = `options-bang.html`).
...
- **Email → Drive automation:** a Google Apps Script (`saveIBKRAttachmentsToDrive`, set up 2026-07-20, lives in Paul's own Google account at script.google.com — not in this repo) auto-saves new IBKR email attachments into the Drive folder above. **Actual observed run time is ~6–7am SGT** (corrected 2026-07-22 — file timestamps in Drive showed `Since_Inception_Detailed_11_20260717.pdf` saved 2026-07-21 ~6:59am SGT and `..._20260720.pdf` saved 2026-07-22 ~6:12am SGT; previously documented as ~7–8pm SGT, which the evidence doesn't match — Paul hasn't confirmed the actual Apps Script trigger setting, so treat ~6–7am SGT as the best-evidence estimate, not a Paul-confirmed fact). So the newest statement is normally already sitting in Drive by early morning — no need for him to manually export/upload each day, and no need to expect anything new until the next morning's run.
...
### IBKR Update Log Rule (Standing Default — added 2026-07-20; index.html wiring changed 2026-07-23)
...
- **⚠️ index.html no longer auto-discovers IB pages (changed 2026-07-23):** Paul asked to consolidate IB01–IB04 into one page with clickable "Page 1"–"Page 4" tabs (`ibkr-update-log.html`). The `loadSection({ prefix: 'IB', ... })` call was **removed** from index.html's `init()`, and the IBKR Update Log section on index.html now shows one static hardcoded link to `ibkr-update-log.html` instead of an auto-generated list. So for every new IB\<next-number\>.html going forward: still create the standalone dated file as before (it's the source of truth / individually shareable/linkable), but **also** manually add a new tab + `.page-panel` to `ibkr-update-log.html` (copy the pattern of the existing 4 pages: add a `<button class="tab-btn" id="tab-N">` to `.tabs-bar`, a matching `<div class="page-panel" id="page-N">` with the article's content, bump the `setPage()`/`setLang()` loop bound from 4 to N, and update the tab count in index.html's `ibklog-status` and the link text). Commit + push the dated IB file, `ibkr-update-log.html`, and index.html together in the same commit.
...
## T29 Options Dashboard — Standing Notes (added 2026-07-23)
- The **Account Value** card (NAV chart, $54,940.59, since-inception %, period buttons) lives only on the **Positions** tab of `options-bang.html`. **Do not duplicate or move it into other tabs** (e.g. the new IC Benefits tab) — Paul confirmed this explicitly when reviewing the IC Benefits tab.
