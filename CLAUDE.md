...
- **Every map must have fullscreen.** ALL maps (route maps, location maps, mini maps) must include a CSS-based fullscreen toggle. Pattern:
  - ⛶ expand button (top-right of map, z-index:1000)
  - On open: set map div to `position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:99998`
  - **Close button: red pill at BOTTOM CENTRE** — `position:fixed;bottom:32px;left:50%;transform:translateX(-50%);z-index:99999;background:#ef4444` — NOT top-right (gets hidden behind iOS header). Label: `✕ Close Map`
  - Call `map.invalidateSize()` after toggling
  - Never use browser `requestFullscreen` API — doesn't work on iOS
