...
- **Email → Drive automation:** a Google Apps Script (`saveIBKRAttachmentsToDrive`, set up 2026-07-20, lives in Paul's own Google account at script.google.com — not in this repo) runs daily ~8–9pm Tokyo time (~7–8pm SGT), searches Gmail for IBKR emails with attachments, and auto-saves new ones into the Drive folder above. So the newest statement is normally already sitting in Drive by the time Paul asks — no need for him to manually export/upload each day.
