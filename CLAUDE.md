...
- **2026-08-06: AAPL single-leg short put OPENED** at 23:52:47 SGT. Sold 1× AAPL Sep 18'26 290 Put @ $3.60 (MEMX exchange), 43 DTE at entry. **Net credit ~$359.38** (after $0.616947 commission). Not a 4-leg Iron Condor — a standalone cash-secured/naked put, same structure as the GOOGL trade above. Order ID 015f0c16.00017253.6a740b6e.0002, Exch. Exec. ID 720594632282801717, Account U19689825. This is the position flagged-but-not-entered on 2026-07-31 (AAPL post-earnings sell-off to ~$302.91) — Paul went with the 1-leg cash-secured put route. Reference price ~$314.70 (search-derived, 6 Aug — live quote fetch blocked in-session, not a confirmed tick) puts the put cushion at ~7.9% ((314.70−290)/314.70), breakeven $286.40. New "AAPL" tab added to T29's Table 660 (Bang Trade Analysis) — Table 1517 (Fee), 1518 (Premium vs Fee), 1519 (P&L), 1520 (Timeline); also added to Table 900 (P&L Summary, open, no mark yet) and Table 8 (Premium by Symbol, $360.00). No IBKR statement mark yet (filled intraday, after any pending PortfolioAnalyst cutoff) — P&L will populate once AAPL appears in a live statement. Logged in `IB23.html`.
- **2026-08-07: POOL covered call OPENED** at 22:19:46 SGT. Sold 1× POOL Sep 18'26 230 Call @ $2.69 (BOX exchange), 42 DTE at entry, against 100 already-held POOL shares (avg cost $278.40). **Net credit ~$267.85** (after $1.151183 commission). Not a 4-leg Iron Condor — a single-leg covered call, same structure as FDS. Order ID 015f0c16.00017253.6a755d07.0001, Exch. Exec. ID S0X58gk00000001, Account U19689825. Live POOL quote at order time ~$206.19 → Call Cushion (230−206.19)/206.19 ≈ 11.5%. **Strike sits $48.40 below cost basis** — if assigned at $230, shares get called away for a realized loss vs the $278.40 cost, cushioned only by the $269 premium (net loss if assigned ≈ −$4,571); this is an income trade, not downside protection. New "POOL" tab added to T29's Table 660 (Bang Trade Analysis) — Table 1527 (Fee), 1528 (Premium vs Fee), 1529 (P&L, no mark yet), 1530 (Timeline); new Gauge card (Table 1531 · Chart 24) and Risk Zone rail card added too, plus `CALL_ONLY_CFG`/`POSITION_STATUS`/`SP_ROW_ID`/`GAUGE_ENTRY_DATE`/`GAUGE_EXPIRY_DATE`/`GAUGE_MOOD_SYMS` wiring and a `coveredCall` field on the Share Ave Cost tab's POOL row. Also added to Table 900 (P&L Summary, open, no mark yet) and Table 8 (Premium by Symbol, $269.00). No IBKR statement mark yet (filled after any pending PortfolioAnalyst cutoff) — P&L will populate once this POOL leg appears in a live statement. Logged in `IB25.html` (added as Page 25 to `ibkr-update-log.html`, T45 card page-range on `tools.html` bumped Page 1-24 → Page 1-25).
- **2026-08-07: ZTS covered call OPENED** at 22:31:35 SGT. Sold 1× ZTS Sep 18'26 90 Call @ $0.38 (BATS exchange), 42 DTE at entry, against 100 ZTS shares held at $135.60 avg cost. **Net credit ~$37.98** (after $0.015867 commission). Order ID 015f0c16.00017253.6a755d3c.0003, Exch. Exec. ID 1959104132974S, Account U19689825. ZTS was $75.89 at the time (reported earnings 6 Aug 2026, sold off hard), so the 90 strike is ~18.6% OTM (Call Cushion (B−A)/A) — a small income trade, same shape as the FDS/POOL covered calls above. Strike sits **well below** the $135.60 cost basis (−33.6%), so assignment would still lock in a real loss on the shares — reviewed the order screenshot before filling and flagged this explicitly per the standing IC Order Screenshot Review Rule. New "ZTS" tab added to T29's Table 660 (Bang Trade Analysis) — Table 1532 (Fee), 1533 (Premium vs Fee), 1534 (P&L, no mark yet), 1535 (Timeline); new Gauge card (Table 1536 · Chart 25) and Risk Zone rail card added too, plus `CALL_ONLY_CFG`/`POSITION_STATUS`/`SP_ROW_ID`/`GAUGE_ENTRY_DATE`/`GAUGE_EXPIRY_DATE`/`GAUGE_MOOD_SYMS` wiring and a `coveredCall` field on the Share Ave Cost tab's ZTS row. Also added to Table 900 (P&L Summary, open, no mark yet) and Table 8 (Premium by Symbol, $37.98). While in this area, filled two gaps left from the POOL addition: POOL was missing its own "Share POOL Link" menu button and `#bang-pool` hash route (now added alongside ZTS's), and several row/underlying counts in the T9 header, "In plain words" note, and Table 8 subtitle were stale at 9/11 instead of reflecting POOL (now corrected to 13 rows / 11 underlyings including both POOL and ZTS). No IBKR statement mark yet (filled after any pending PortfolioAnalyst cutoff) — P&L will populate once ZTS appears in a live statement.
- **2026-08-07: DDOG bull put spread OPENED** at 23:57:17 SGT. Sold 1× DDOG Sep 18'26 210 Put @ $6.61 (commission $1.15) / Bought 1× DDOG Sep 18'26 195 Put @ $3.36 (commission $1.13), both CBOE. **Net credit ~$322.72** (($661−$1.15) − ($336+$1.13)), $15 wide wing, 42 DTE at entry. Max Loss ~$1,177.28, Max Return ~$322.72 (ratio ~3.65:1). First 2-leg vertical (put credit spread, not a 4-leg Iron Condor or naked single put) opened on this dashboard — chosen specifically to fix a bad risk/reward ratio: an earlier 200/190 version of this same trade (order-ticket-only, never filled) priced off the Ask showed a 10.7:1 max-loss:max-win ratio, which Paul flagged as a concern; widening to 210/195 and getting a real fill near mid (rather than the Ask) brought it down to ~3.65:1. DDOG itself was flagged same-day as a post-earnings IV-crush candidate — beat Q2 (rev +36% YoY) but sold off ~15-19% on guidance flagging slower spend from its largest AI customer (OpenAI), a sentiment/customer-concentration reaction rather than core deterioration; already reported (6 Aug), so no earnings-gap risk left this cycle. Reference price ~$228.82 (search-derived at order time, not a confirmed tick) puts the put cushion at ~8.2% ((228.82−210)/228.82), breakeven ~$206.77. Logged in `IB26.html` (added as Page 26 to `ibkr-update-log.html`, T45 card page-range on `tools.html` bumped Page 1-25 → Page 1-26) and added to T29 Table 900 (P&L Summary, open, no mark yet) and Table 8 (Premium by Symbol, $322.72, header count corrected eleven→twelve). **Scope note:** unlike the full per-symbol Bang tab build-out done for POOL/ZTS (dedicated Fee/Premium/P&L/Timeline tables + Gauge/Rail cards), DDOG only got the Table 900 + Table 8 entries this round — a 2-leg vertical spread doesn't fit the existing naked-put/covered-call/4-leg-IC templates cleanly, so the fuller Bang-tab visualization was deliberately deferred rather than force-fit; flagged to Paul, offered as a follow-up if wanted.
...
- **Paul's default Gauge tab view is "Hide Label" mode (Standing Default — added 2026-08-07).** Recognize it in a screenshot by: the mini-cushion diamond/warning badges ARE visible under each gauge, but the legend, cushion-formula/value rows, and Var readouts are NOT. Assume every Gauge-tab screenshot Paul sends is this mode unless it clearly shows the full-detail layout instead — don't wait for him to say so.
- **Multi-mode CSS check before shipping any new Gauge-tab element (Standing Default — added 2026-08-07, learned the hard way — a new P&L label was built, worked in the default view, then turned out invisible because it landed inside `#gaugeGrid.hide-labels`'s hidden set, which is Paul's actual default view above):** `options-breadboard.html` has several display-mode overrides that hide/shrink/reposition elements inside `#gaugeGrid` — the `#gaugeGrid.hide-labels {...}` block and the phone-landscape `@media (orientation:landscape) and (max-height:600px)` block are the two big ones. Whenever adding, moving, or resizing anything inside `#gaugeGrid` (or any other component with its own mode-toggle CSS), grep the file for every existing mode block touching that component and explicitly decide/add a rule for the new element in each one — never leave it to silently inherit undefined behavior in a mode you didn't check.
- **Render and screenshot before claiming a visual change is done (Standing Default — added 2026-08-07):** this environment's network access to `paulsworld.vercel.app` is blocked, so the live site can't be checked directly after pushing. Before saying a UI/CSS change is complete, render the actual file locally (a local static server + Playwright, screenshotting both the default view and Hide Label mode for anything in `#gaugeGrid`) instead of reasoning from the CSS alone — that's what would have caught both the D-badge overlap and the hidden label before Paul had to send a third screenshot.
...
- **Reaffirmed 2026-08-08:** also covers research/analysis steps within a question — e.g. pulling the latest IBKR statement from Drive, checking a live quote, or fetching web data to answer an IC cushion/close/roll question. Just pull whatever data is needed and give the answer/recommendation directly — don't pause mid-analysis to ask if it's OK to check something. (Actually executing a trade is still Paul's own manual action on IBKR — this covers the research and recommendation, not placing the order.)
