...
- **Every map must have fullscreen.** ALL maps (route maps, location maps, mini maps) must include a CSS-based fullscreen toggle (⛶ button top-right → `position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:10000` + ✕ close button). Call `map.invalidateSize()` after toggling. Never use browser `requestFullscreen` API — doesn't work on iOS.
